...
class Jmesse_View_AdminFairSearch extends Jmesse_ViewClass
{
	...
	function preforward()
	{
		...
		// 開催地（地域）
		$this->af->setApp('region', $this->backend->getManager('JmCodeM')->getRegionListSearch());

		// 開催地（国・地域）
		if (null != $this->af->get('region') && '' != $this->af->get('region')) {
			$this->af->setApp('country', $this->backend->getManager('JmCodeM')->getCountryListSearch($this->af->get('region_jp')));
		} else {
			$this->af->setApp('country', $this->backend->getManager('JmCodeM')->getCountryListSearch('001'));
		}
