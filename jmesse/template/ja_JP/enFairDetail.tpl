	...
	function answer(url) {
		if($("input[name=a_id]").filter(":checked").val()){
			...
			$.ajax({
		        url: url,
		        type: "GET",
		        data: {action_fairDetailQuestionnaire : true, a_id : $("input[name=a_id]").filter(":checked").val(), lang : "E"},
		        beforeSend: function(){
		        ...
		        success: function(response) {
					if(response.trim()=='success'){
						$("#question_area").hide();
						$("#thank_you_area").show();
						$("#thank_you").html("<br />&nbsp<br /><span><b>Thank you.</b></span><br />&nbsp");
					} else {
						//エラーの場合はなにもしない
			...
			{if ('2' == $app.fair_detail.jecc_flag)}
				<div class="elem_pic">
					<div class="right elem_pic_pad">
						...
						<p class="pic_caption">
							<a href="/en/database/j-messe/jecc/" />Third-party Certification System for Exhibition Statistics</a>
						</p>
					</div>
				</div>
			{/if}
							...
							<ul>
								<div class="elem_paragraph">
									<center>
										<p class="center text">
											<br />
											{$app.fair_qa[0].q_en}<br /><br />
											{section name=it loop=$app.fair_qa}
												<label>
													<input name="a_id" id="a_id" value="{$app.fair_qa[it].a_id}" type="radio"/>{$app.fair_qa[it].a_en}&nbsp;&nbsp;
												</label>
											{/section}
											<br /><br />
											<div style="width:10%" class="linkBox">
												<button type="button" id="questionnaire" onclick="answer('{$config.url}')"><span>Submit</span></button>
											</div>
